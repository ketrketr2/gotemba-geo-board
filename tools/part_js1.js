...
/* 実測モード: aggregate_board.py が実測で上書きした指標のカードだけ tagM=実測 に切替える。
   未実測のカード（語られ指数・レーダー・マトリクス・ペルソナ・時系列等）は tagS のまま。 */
const MEAS=!!META.measured;
const tagM=MEAS?tagR:tagS;
if(MEAS){
 const rb=document.querySelector('.ribbon span:last-child');
 if(rb)rb.innerHTML=`<b>R1 実測ラウンド ${esc(META.round_date||'')}</b>｜AI計測系は<b>実測値</b>（${fmt(META.n_cells)}回答・実費$${(+META.round_cost||0).toFixed(2)}）｜<b>サンプル</b>タグは実測対応前の設計プレビューのみ`;
 const op=document.querySelector('#rsel option');
 if(op)op.textContent=`R1｜実測（${META.round_date||''}）`;
}
let TMRS=[];
...
if(MEAS){
 const fb=S.firstBar||[[['—'],0],[['—'],0]];
 FEED[1]=['実測',`R1計測: 第一想起は<b>${esc(fb[0][0])}が${fb[0][1]}回</b>で首位（2位 ${esc(fb[1][0])} ${fb[1][1]}回）`];
 FEED[3]=['実測',`G2言及率 <b>${S.g2}%</b>・G3第一想起率 <b>${S.g3}%</b>（${fmt(META.n_cells)}回答・実費$${(+META.round_cost||0).toFixed(2)}）`];
 FEED[5]=['実測',`AI引用元に占める自社公式は<b>${S.official}%</b>・UGC系${S.ugc}% — 外部露出が言及率のレバー`];
}
...
function buildNav(){
 ...
 VIEWS.forEach(([id,ic,label,kind])=>{
  if(id==='v2')h+='<div class="sec">AI計測'+(MEAS?'':'（サンプル）')+'</div>';
...
function rV1(){
 ...
 let h=`<div class="crumb rv"><h2>サマリー<small>WEEKLY PULSE</small></h2>
  ...
  <span class="chip glt">${MEAS?('R1 実測 '+esc(META.round_date||'')):'R0 プレビュー'}</span>
 ...
 h+=`<div class="hero">
  ...
  <div class="card rv">
   <div class="ct">ファネル指標 G1–G4${tagM}<button class="q" onclick="help('funnel')">?</button></div>
   <div class="ktiles">
    <div class="kt"><div class="t"><i style="background:var(--ac)"></i>G1 露出機会</div><div class="v mono"><span data-cnt="${S.g1}" data-dec="1">0</span><small> 引用/クエリ</small></div><div class="s">1クエリあたりの<b>期待引用数</b><br>= ${S.g1sub}</div>${MEAS?'':spark([3.2,3.3,3.1,3.4,3.5,3.5,3.6,3.7],76,26,'#FF7A59',1)}</div>
    <div class="kt"><div class="t"><i style="background:var(--bl)"></i>G2 言及率</div><div class="v mono"><span data-cnt="${S.g2}" data-dec="1">0</span><small>%</small></div><div class="s">出現期待 ${S.g2n} で言及</div>${MEAS?'':spark(S.ts.g2.slice(-8),76,26,'#4E9BE8',1)}</div>
    <div class="kt"><div class="t"><i style="background:var(--pu)"></i>G3 第一想起率</div><div class="v mono"><span data-cnt="${S.g3}" data-dec="1">0</span><small>%</small></div><div class="s">言及${S.g3n}中で最初に登場</div>${MEAS?'':spark([38.2,39,38.5,40.1,40.8,40.2,41,41.5],76,26,'#B48CFF',1)}</div>
    <div class="kt"><div class="t"><i style="background:var(--cy)"></i>G4 検索需要</div><div class="v mono"><span data-cnt="${S.g4}">0</span><small> pt</small></div><div class="s">指名検索指数（アンカー=100）${MEAS?'':`<span class="delta up">${S.g4d}</span>`}</div>${MEAS?'':spark(S.ts.dem.slice(-8),76,26,'#22C7D6',1)}</div>
   </div>
 ...
 h+=`<div class="card rv" style="margin-top:14px">
  <div class="ct">AI4面ステータス${tagM}<button class="q" onclick="help('surface')">?</button></div>
...
function rV2(){
 ...
 h+=`<div class="card rv"><div class="ct">面別スコア — G2言及率 / G3第一想起率 / 回答生成率${tagM}</div>
 ...
 h+=`<div class="g g4c" style="grid-template-columns:repeat(4,minmax(0,1fr));margin-top:14px">`+
  S.faces.map(f=>`<div class="face rv" style="--fc:${f.c}">
   <div class="nm"><i></i>${f.n}<span class="tag ${MEAS?'real':'smp'}" style="margin-left:auto">${MEAS?'実測':'SMP'}</span></div>
   <div class="met">
 ...
 h+=`<div class="g g2c" style="grid-template-columns:1.35fr 1fr;margin-top:14px">
  <div class="card rv"><div class="ct">引用元ドメインランキング（4面合算）${tagM}</div>
   ...
   <div class="sub2" style="margin-top:8px">上位${S.domains.length}ドメイン計 ${S.domains.reduce((a,d)=>a+d[1],0).toFixed(1)}%（残りはロングテール）。Geminiのリダイレクタ引用はドメイン復元済み。</div>
  </div>
  <div class="card rv"><div class="ct">引用の構成 — 公式は思ったより引かれない${tagM}</div>
...
function rV3(){
 ...
 h+=`<div class="g g2c" style="grid-template-columns:1.05fr 1fr">
  ...
  <div class="card rv"><div class="ct">第一想起の獲得数（${MEAS?('出現期待'+String(S.g2n).split('/')[1]+'中'):'302回答中'}）${tagM}</div>
   ...
   <div class="sub2" style="margin-top:8px">${MEAS?`<b>${esc(S.firstBar[0][0])} ${S.firstBar[0][1]} vs ${esc(S.firstBar[1][0])} ${S.firstBar[1][1]}</b> — 御殿場が言及された回答のうち${S.g3}%で最初に登場（${S.g3n}）。`:'残り30回答はTier2施設・その他。<b>御殿場73 vs 木更津54</b> — 差は19回答で、家族・実用文脈で侵食が進むと逆転圏。'}</div>
  </div>
 </div>`;
